...
public class GraphicLifeCounter extends GameObject {
    ...
    public GraphicLifeCounter(Vector2 widgetTopLeftCorner, Vector2 widgetDimensions, Counter livesCounter,
        ...
        int objLocationX = (int)(widgetTopLeftCorner.x() + HALF * widgetDimensions.x());
        int objLocationY = (int)(widgetTopLeftCorner.y() - HALF * widgetDimensions.y());

        // creates the private array of hearts and places them by their center
        createGameObjectsArray(numOfLives);
        for (int i = 0; i < numOfLives; i++) {
            gameObjects[i] = new GameObject(widgetTopLeftCorner, widgetDimensions, widgetRenderable);
            gameObjects[i].setCenter(new Vector2(objLocationX + i * widgetDimensions.x(), objLocationY));
